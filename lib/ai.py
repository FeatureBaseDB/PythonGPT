...
def gpt_chat_complete(words, keyterms, title, question):
	try:
		messages = [
			{"role": "system", "content": "Document title: %s\nInstructions: Answer the question below from the document fragments." % title},			
			{"role": "user", "content": "Keyterms: " + ", ".join(keyterms) + "\nFragments:\n'''"+words+"'''\nQuestion: "+question}
		]
...
# gptchat
def gpt_chat_code(words, keyterms, title, question):
	try:
		messages = [
			{"role": "system", "content": "Document title: %s\nInstructions: You write Python code that will be run for the user." % title},			
			{"role": "user", "content": "Keyterms: " + ", ".join(keyterms) + "\nFragments:\n'''"+words+"'''\nQuery to convert to Python code: "+question}
		]
...
@model
def gpt_codeterms(document):
	# load openai key then drop it from the document
	openai.api_key = document.get('openai_token')
	document.pop('openai_token', None)

	# substitute things
	template = load_template("get_codetandqs")
	prompt = template.substitute(document)
	
	ai_dict = gpt3_dict_completion(prompt)

	document.setdefault('keyterms', ai_dict.get('keyterms'))
	document.setdefault('explanation', ai_dict.get('explanation'))
	document.setdefault('error', ai_dict.get('error', None))
...
@model
def ask_gptcode(document):
	# load openai key then drop it from the document
	openai.api_key = document.get('openai_token')
	document.pop('openai_token', None)

	answer = gpt_chat_code(document.get('text'), document.get('keyterms'), document.get('title'), document.get('question'))
